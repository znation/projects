...
main = do
	...
	let qs = makeQuotes (init (mywords (reverse mylines)))
	let numseeds = 40
...
generation 899 states days _ = best 20 states (quotes(head days))
...
replaceSeedWithRandom :: [Int] -> TradeState -> TradeState
replaceSeedWithRandom randomInts s = TradeState (cash s) (shares s) (pricePaid s) (randomSeed randomInts)
...
data Quote = Quote {symbol :: String, price :: Double, date :: String}
			deriving (Show)
instance QC.Arbitrary Quote where
	arbitrary = do
		...
		b <- QC.arbitrary
		c <- QC.arbitrary
		return (Quote a (abs b) c)
instance QC.Arbitrary Char where
    arbitrary     = QC.elements (['A'..'Z'] ++ ['a' .. 'z'])
newtype Day = Day {quotes :: [Quote]}
...
makeQuote (x:y:z) = Quote x (read y::Double) (unwords z)
makeQuote _ = Quote "Invalid" 0.0 "Invalid"
...
medianState :: [TradeState] -> TradeState
medianState s = (sort s)!!((length s) `div` 2)